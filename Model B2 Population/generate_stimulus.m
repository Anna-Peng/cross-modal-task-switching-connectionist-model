...
while stimulus_change==1
    ...
    if strcmpi(call_condition(condition), 'M')
        switch samestimuli
            case 4 % Uniselection stimuli
%                     if (~isequal(current_vis,current_task) && prob<0.6) && (~isequal(current_aud,current_task) && prob<0.6) % if the stimulus elements are not from target set and should be within prob                    
%                         stimulus_change=1;
%                     elseif (isequal(current_vis,current_task) && prob>0.6) || (isequal(current_aud,current_task) && prob>0.6)
%                         stimulus_change=1;
                    if sum(current_vis+current_aud)>1
                        if omit==1
                            current_vis=[0 0];
                        elseif omit==2
                            current_aud=[0 0];
                        ...
                        stimulus_change=0;
                    elseif sum(current_vis+current_aud)==0
                        stimulus_change=1;
                    else
                        stimulus_change=0;
                    end
