    ...
    case 'M'
            ...
            for j=1:vtask_num
                    ...
                    for mod=1:2 % no of modalities
                        if PS_node(j,mod)~=0
                            if mod==1 %k<interval && mod==1
                                vStep_mix=vStep_v;
                            elseif mod==2 %k<interval && mod==2
                                vStep_mix=vStep_a;
%                             else
%                                 vStep_mix=vStep;
                            end
                        totalinput=PS_netinput(j)+Top2PS_net(j,mod);
                        PS_update=eta(totalinput, PS_node(j,mod), vStep_mix, vminPSact, vmaxact);
...
%                 interval=vinterval_rep;
    ...
    %                                 vStep_mix=vStep;
                            ...
                            PS_update=eta(PS_netinput(j), PS_node(j,mod), vStep_mix, vminPSact, vmaxact);
                            PS_node(j,mod)=activation(PS_node(j,mod), PS_update, vnoise_sd, vminPSact, vmaxact);
                            end
                        end
                    end
