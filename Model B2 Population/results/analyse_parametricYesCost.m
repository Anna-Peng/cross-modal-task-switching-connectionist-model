...
file2load=sprintf('zwithinstatsYes%s.mat', whichdatefile);
load(file2load);

datefile=strcat('zcomparemodelsYes', whichdatefile, '.mat');
...
[Parameter.Pure, Parameter.Mixed]=deal(Pure.Parameter(1,:));
...
PureParameter=cell2mat(Pure.Parameter(Index(:,k)==1,2:end));
MixParameter=cell2mat(Repetition.Parameter(Index(:,k)==1,2:end));
Parameter.Pure(k+1,:)=[network_type(k),num2cell(PureParameter(1,:))];
Parameter.Mixed(k+1,:)=[network_type(k),num2cell(MixParameter(1,:))];
...
save(datefile, 'RT','Error', 'RTCI', 'ErCI', 'Parameter','whichdatefile');
