...
locaRT=20;%str2num(locaRT);
colorselect=1;%input('[input color, green=1, blue=2, red=3] '); 
toplimit=100;%input('[ylimit] ');
PlotAgeorCon=3;%input('Plot Age Group or network update probablity? Age=1, Probability=2, Top-Down=3 ');
...
%         y1CI(:,ii)=[RTCI.Pure(indexA(:,ii),locaRT);RTCI.Rep(indexA(:,ii),locaRT);RTCI.Swi(indexA(:,ii),locaRT)];
        y1(:,ii)=[;RT.Rep(indexA(:,ii),locaRT);RT.Swi(indexA(:,ii),locaRT)];
        y1CI(:,ii)=[RTCI.Rep(indexA(:,ii),locaRT);RTCI.Swi(indexA(:,ii),locaRT)];
        else
...
%    set(gca,'XTickLabel',{'Mixing Cost', 'Switch Cost'});
   ...
   set(findall(h, '-property', 'FontSize'), 'FontSize', 14);%, 'fontWeight', 'bold')
...
%     label data
    labels = arrayfun(@(value) num2str(value,'%2.0f'),repmat(probability(ib),1,size(y1,1)),'UniformOutput',false);

%     labels = arrayfun(@(value) num2str(value,'%2.0f'),y1(:,ib),'UniformOutput',false);
    text(xData,zeros(1,size(y1,1))-gap,labels,'HorizontalAlignment','center',...
    'rotation', -60, 'FontSize', 12, 'VerticalAlignment','middle');    

% text(xData,y1(:,ib)+gap,labels,'HorizontalAlignment','center',...
%     'rotation', 0, 'FontSize', 10, 'VerticalAlignment','middle');
   end
