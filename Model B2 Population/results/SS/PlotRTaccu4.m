...
regress=input('[Apply regression model? Yes=1, No=0]'); 
if regress==1
    pureconstant=input('Pure constant]'); 
    purecoef=input('Pure Coefficient]'); 
    mixconstant=input('Mix constant]'); 
    mixcoef=input('Mix Coefficient]'); 
end
...
for i=1:length(analysefile) legendname{i}=input('[Enter legend name] ','s'); end
    ...
    if regress==1
    y1(1,:)=y1(1,:).*purecoef+pureconstant;
    y1(2:3,:)=y1(2:3,:).*mixcoef+mixconstant;
    y1CI(1,:)=y1CI(1,:).*purecoef;
    y1CI(2:3,:)=y1CI(2:3,:).*mixcoef;
    tickinterval=100;
    maxtick=2000;
    regressname='regress';
    else
    ...
    box off
    ylim(AX(1),[0 400]);%y1max*1.7])
    ylim(AX(2),[0 1.4])
    set(AX,'ycolor', 'k');
    
    set(AX(1),'ytick', 0:tickinterval:maxtick)
...
%     'rotation', rot, 'FontSize', 10, 'VerticalAlignment','middle');
   end
...
%    title(titlename)
   ...
   set(findall(h, '-property', 'FontSize'), 'FontSize', 16);%, 'fontWeight', 'bold')


   figurename=strcat(titlename, whichdatefile, regressname,'.png');
