...
whatRT=input('[Plot RT or Accuracy? Accuracy=1, RT=2] ');
if whatRT==1
    yname='Accuracy';
else
    yname='Time (cycles)';
...
for age=1:3   
    ...
    for j=1:3 % condition
        ...
        hold on
        plot([x1(end)+0.5 x1(end)+0.5], [0 maxy],'k-', 'Linewidth', 1);
...
if whatRT==1
  ...
  for i=1:3   
  t(i)=annotation('textbox',dim(i,:), 'String',legendname(i), 'FontSize', 15);%,'FitBoxToText','on');
