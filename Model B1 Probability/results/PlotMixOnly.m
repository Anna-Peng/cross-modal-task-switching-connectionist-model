...
addpath(f)
whichdatefile='YesB2-14';%input('[input date file] ','s'); 
...
whatmeasure=1;%1=probability, 2=top-down signal
if whatRT==1
    yname='Accuracy';
elseif whatRT==2
    yname='Reaction Time (cycles)';
elseif whatRT==3
    yname='Priming Effect in RT (cycles)';
end
...
for age=1:3   
    ...
    plot([0 x1(end)+4], [0 0],'k-', 'Linewidth', 1);
...
elseif whatRT==3
    ylim([-40 90])
    set(gca,'ytick', -40:20:90)
end
 
  set(gca,'Xtick',1:length(tickname)*2)
  set(gca,'XTickLabel',tickname);
  colorname='brg';
  
  dim=[0.66 0.4 0.5 0.5; 0.66 0.36 0.5 0.5; 0.66 0.32 0.5 0.5];
  for i=1:3   
