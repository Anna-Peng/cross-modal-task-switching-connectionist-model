...
xname='Update Probability %'; %input('[input trial x-axis label] ', 's'); 
if whatRT==1
    locaRT=2;
    type='allRT';
elseif whatRT==2
...
%     end
    ...
    'rotation', rot, 'FontSize', 8, 'VerticalAlignment','middle', 'parent',AX(2));

    hold(AX(1), 'on');
    errorbar(AX(1), y1,y1CI,'k.', 'LineWidth', 1)

    labels = arrayfun(@(value) num2str(value,'%2.0f'),y1,'UniformOutput',false);
    text(xData,y1+10,labels,'HorizontalAlignment','center',...
