    ...
    switch vcondition
        case 'M'
            for n=1:vtask_num
                ...
                task_li_node(n)=0;
                if vlateral_in==0 % if lateral inhibition from inhibited nodes are turned off
...
switch vcondition
    case 'M'
            ...
            for j=1:vtask_num
                ...
                task_netinput(j)=vtask_control(j)+ PS2TAprime(1,j) - task_li + vtask_bias+PS2TA(j); % netinput to task node (j)
    ...
    case 'P'
                ...
                task_netinput(1,1)=vtask_control(current_task==1)+ PS2TAprime(1,current_task==1) + vtask_bias+PS2TA(1,current_task==1); % netinput to task node (j)
...
%                     RO_outer_li_node(RO_outer_li_node<0)=0;
                    RO_outer_li=sum(RO_outer_li_node.* -vli_weight); % using the same value of lateral inhibition for all weights between response sets
                        switch vcondition
                            case 'M'
                            net(i,j)=PS2ROnet(i,j) + TA2ROnet(i,j)+ RO_outer_li+RO_inner_li+ voutput_bias;
                            case 'P'
                            if update(i,j)==1            
                                net(i,j)=PS2ROnet(i,j) + TA2ROnet(i,j)+ +RO_inner_li+ voutput_bias; % netinput to the other RO is turned off=0
                            else
                                net(i,j)=0;
                            end
                        end
...
modality_record=[modality_record(2),target_modality];
    ...
    zall_data(end+1,:)={counter, type, TAname(current_task==1), char(current_stimulus), target_modality,...
        ...
        task_netinput(1), task_netinput(2),...
        PS_record,t};
        counter=counter+1;
