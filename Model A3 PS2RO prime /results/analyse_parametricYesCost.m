...
PureCI=1.96* std(PureRT, 0, 1)/sqrt(size(PureRT,1));
...
RepCI=1.96* std(RepRT, 0, 1)/sqrt(size(RepRT,1));
...
PureErCI=1.96* std(PureError, 0, 1)/sqrt(size(PureError,1));
PureError=nanmean(PureError,1);
RepError=cell2mat(Repetition.Error(Index(:,k)==1,2:end));
RepErCI=1.96* std(RepError, 0, 1)/sqrt(size(RepError,1));
RepError=nanmean(RepError,1);
SwiError=cell2mat(Swi.Error(Index(:,k)==1,2:end));
SwiErCI=1.96* std(SwiError, 0, 1)/sqrt(size(SwiError,1));
