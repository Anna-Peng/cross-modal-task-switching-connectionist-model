...
vnblock= input('no. of blocks (8 trials in each block) (5 blocks x 8= 40 trials in each condition): '); 
call_condition = input('Condition P=pure, M=mixed (default "Pure Mixed Mixed Mixed") \nEnter for default: ', 's');
...
vselectiontime=input('Early Selection (inh)=1, Late Selection (ex)=2');
% vselectiontime=1; % early selection or late selection
%% if using the same stimuli throughout
samestimuli=input('[repeat the same stimulus=1 unique=2 or random=3] ');
...
if isempty(vreadfile)
        ...
        zparameter=TXT(1,2:size(inputanswer,2)+1);
        inputcell=num2cell(inputanswer);
        zparameter=[zparameter;inputcell];
...
%% Stimulus presentations
            ...
            PS_node=[current_vis',current_aud'].*vstimulus_input; % PS nodes initiation
            ...
            PS2TAprime(current_task==1)=sum((PSvis_TA_prime(:,current_task==1)'.*current_vis)+(PSaud_TA_prime(:,current_task==1)'.*current_aud));
            PS2TAprime(current_task==0)=sum((PSvis_TA_prime(:,current_task==0)'.*current_vis)+(PSaud_TA_prime(:,current_task==0)'.*current_aud));% N-1 connection x current stimulus to dog TA
            PS2ROprime_all=PSvis_RO_prime*current_vis'+PSaud_RO_prime*current_aud';
...
%             [PSrecordall, PSupdaterecordall]=deal(zeros(1,4));
...
switch vcondition
    case 'M'
            ...
            elseif select_attention==1 && vselectiontime==1  % if it is early selection (inhibition, PS input=1)
                PS_netinput=task_node(end,:).*-TA2PSweight;
                PS_netinput=fliplr(PS_netinput);
            else PS_netinput=[0 0]; 
            end
            ...
            for j=1:vtask_num
                    ...
                    for mod=1:2 % no of modalities
                        if PS_node(j,mod)~=0 && TA2PSweight~=0
                        ...
                        PSrecord(j,mod)=PS_update; %% to be deleted
                        end   
                    end
            end
            ...
            for j=1:vtask_num
                ...
                if vlateral_in==0 % if lateral activity from inhibited nodes are turned off
                    task_li_node(task_li_node<0)=0; %% turn off the inhibition from inhibited node
                end
    ...
    case 'P'
                ...
                PS_netinput=task_node(end,current_task==1).*TA2PSweight;
                    if PS_node(current_task==1,mod)~=0 && TA2PSweight~=0 %only update the task-associated PS
                ...
                for i=1:2 % cycling through response sets
                    ...
                    if vlateral_in==0 % if lateral activity from inhibited nodes are turned off
                    RO_inner_li_node(RO_inner_li_node<0)=0; %% turn off the inhibition from inhibited node
                    end
                    ...
                    if j==1
                        ...
                        inner_li=vli_weight; % same lateral inhibition weight as others
                    end
...
%                 PSupdaterecordall(t,:)=[PSrecord(1,:), PSrecord(2,:)]; %% 1st row dog response 2nd row bird response
%                 PSrecordall(t,:)=[PS_node(1,:), PS_node(2,:)];
                t=t+1;
...
PSvis_TA_prime=current_vis'*primedNode*vpriming; % 2 x 2 column task to PS connections (dog-DA; dog-BA) & (bird-DA; bird-BA)
PSaud_TA_prime=current_aud'*primedNode*vpriming;
PSvis_RO_prime=ROnode'*current_vis*vROpriming; % 4x2 matrix [DY;DN;BY;BN]*[DV,BV]
PSaud_RO_prime=ROnode'*current_aud*vROpriming;
% task_record=task_node; %% to be deleted
...
modality_record=[modality_record(2),target_modality];
    ...
    zall_data(end+1,:)={counter, type, TAname(current_task==1), char(current_stimulus), target_modality,...
        ...
        PS_record,t,net_dog(1,1), net_dog(1,2), net_bird(1,1), net_bird(1,2)};% PS2ROprime(1,1),PS2ROprime(1,2), PS2ROprime(2,1), PS2ROprime(2,2)};
        counter=counter+1;
...
name=sprintf('%s_%d',filename{no_s*2,:},vsubject);
current_parameter=[zparameter(1,:);zparameter(no_s*2:no_s*2+1,:)];
...
% k=['ref';filename];
% zparameterrecord=[k,zparameter];
% 
% save('parametersetting','zparameterrecord')
